...
public class OpenAiAssistantHelper(string apiKey, string model)
{
    ...
    public async Task UploadFilesToVectorStoreAsync(string folderPath)
    {
    ...
    public async Task CreateAssistantAsync()
    {
        ...
        request.AddJsonBody(new
        {
            model,
            instructions =
                "You are a healthcare integration expert. Use the uploaded documentation and examples to generate compliant HL7 messages." +
                " Follow the structure and field requirements found in the provided files." +
                @" Return only the HL7 message. Format each segment on its own line. Use carriage return and line feed (CRLF) WINDOWS, but not the string '\r\n'." +
    ...
    private async Task DeleteAllVectorStoreAsync()
    {
        ...
        foreach (var vectorStore in json.RootElement.GetProperty("data").EnumerateArray())
        {
            ...
            await DeleteVectorStoreAsync(id);
        }
    }
}